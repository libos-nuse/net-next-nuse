...
int sim_sock_socket(int domain, int type, int protocol,
...
ssize_t sim_sock_recvmsg(struct SimSocket *socket, struct msghdr *msg,
			 int flags)
{
	struct socket *kernel_socket = (struct socket *)socket;
...
int sim_sock_getsockname(struct SimSocket *socket, struct sockaddr *name,
			 int *namelen)
{
...
int sim_sock_getpeername(struct SimSocket *socket, struct sockaddr *name,
			 int *namelen)
{
	struct socket *sock = (struct socket *)socket;
...
int sim_sock_bind(struct SimSocket *socket, const struct sockaddr *name,
		  int namelen)
{
	struct socket *sock = (struct socket *)socket;
...
int sim_sock_connect(struct SimSocket *socket, const struct sockaddr *name,
...
int sim_sock_accept(struct SimSocket *socket, struct SimSocket **new_socket,
		    int flags)
{
...
static int sim_pollwake(wait_queue_t *wait, unsigned mode, int sync, void *key)
{
	struct sim_ptable_entry *entry =
		(struct sim_ptable_entry *)wait->private;
...
static void sim_pollwait(struct file *filp, wait_queue_head_t *wait_address,
			 poll_table *p)
{
	struct poll_wqueues *pwq = container_of(p, struct poll_wqueues, pt);
	struct sim_ptable_entry *entry =
		(struct sim_ptable_entry *)
		sim_malloc(sizeof(struct sim_ptable_entry));
...
void sim_sock_poll(struct SimSocket *socket, struct poll_table_ref *ret)
{
	...
	if (ret->opaque) {
		ptable =
			(struct poll_wqueues *)sim_malloc(sizeof(struct
								 poll_wqueues));
		if (!ptable)
			return;
...
void sim_sock_pollfreewait(void *polltable)
{
	...
	if (ptable && ptable->table) {
		struct sim_ptable_entry *entry =
			(struct sim_ptable_entry *)ptable->table;
