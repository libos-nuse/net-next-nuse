...
int socket(int v0, int v1, int v2)
{
	...
	ret = sim_sock_socket(v0, v1, v2, &kernel_socket);
	if (ret < 0)
		errno = -ret;
...
int close(int fd)
{
	...
	sim_update_jiffies();
	ret = sim_sock_close(nuse_fd_table[fd].kern_sock);
	if (ret < 0)
		errno = -ret;
...
ssize_t recvmsg(int fd, const struct msghdr *v1, int v2)
{
	...
	ret = sim_sock_recvmsg(kernel_socket, v1, v2);
	if (ret < 0)
		errno = -ret;
	sim_softirq_wakeup();
	return ret;
...
ssize_t sendmsg(int fd, const struct msghdr *v1, int v2)
{
	...
	ret = sim_sock_sendmsg(kernel_socket, v1, v2);
	if (ret < 0)
		errno = -ret;
	sim_softirq_wakeup();
	return ret;
...
extern int sim_sock_getsockname(struct socket *, struct sockaddr *name,
				int *namelen);
int getsockname(int fd, struct sockaddr *name, int *namelen)
{
	...
	ret = sim_sock_getsockname(kernel_socket, name, namelen);
	sim_softirq_wakeup();
	return ret;
}

extern int sim_sock_getpeername(struct socket *, struct sockaddr *name,
				int *namelen);
...
extern int sim_sock_bind(struct socket *, const struct sockaddr *name,
			 int namelen);
int bind(int fd, struct sockaddr *name, int namelen)
{
	...
	ret = sim_sock_bind(kernel_socket, name, namelen);
	sim_softirq_wakeup();
	return ret;
}
...
int connect(int fd, struct sockaddr *v1, int v2)
{
	...
	ret = sim_sock_connect(kernel_socket, v1, v2, 0);
	sim_softirq_wakeup();
	return ret;
}
...
int listen(int fd, int v1)
{
	...
	retval = sim_sock_listen(kernel_socket, v1);
	sim_softirq_wakeup();
	return retval;
}
#if 0
...
extern int sim_sock_accept(struct socket *socket, struct socket **new_socket,
			   int flags);
int accept4(int fd, struct sockaddr *addr, int *addrlen, int flags)
{
	...
	retval = sim_sock_accept(kernel_socket, &new_socket, flags);
	if (retval < 0) {
		errno = -retval;
		sim_free(new_socket);
		printf("accept err\n");
	       ...
	       const void *optval, int optlen)
{
	sim_update_jiffies();
	struct socket *kernel_socket = nuse_fd_table[fd].kern_sock;
	int retval = sim_sock_setsockopt(kernel_socket, level, optname, optval,
					 optlen);
	if (retval < 0) {
		errno = -retval;
		sim_softirq_wakeup();
	       ...
	       void *optval, int *optlen)
{
	sim_update_jiffies();
	struct socket *kernel_socket = nuse_fd_table[fd].kern_sock;
	int retval = sim_sock_getsockopt(kernel_socket, level, optname, optval,
					 optlen);
	if (retval < 0) {
		errno = -retval;
		sim_softirq_wakeup();
