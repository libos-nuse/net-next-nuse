...
struct SimTask *sim_task_create(void *private, unsigned long pid)
{
	...
	cred = sim_malloc(sizeof(struct cred));
	if (!cred)
		return NULL;
	/* XXX: we could optimize away this allocation by sharing it
	   for all tasks */
	ns = sim_malloc(sizeof(struct nsproxy));
	if (!ns)
		return NULL;
	user = sim_malloc(sizeof(struct user_struct));
	if (!user)
		return NULL;
	info = alloc_thread_info(&task->kernel_task);
	if (!info)
		return NULL;
	kpid = sim_malloc(sizeof(struct pid));
	if (!kpid)
		return NULL;
			  ...
			  void *key)
{
	...
	struct SimTask *sim_task = container_of(task, struct SimTask,
						kernel_task);
